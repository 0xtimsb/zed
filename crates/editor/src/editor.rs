...
impl Editor {
    ...
    pub fn toggle_inline_completions(
        ...
        if self.show_inline_completions_override.is_some() {
                ...
                self.buffer.read(cx).text_anchor_for_position(cursor, cx)
            {
                ...
                self.set_show_inline_completions(Some(show_edit_predictions), window, cx);
            }
        }
    }
    ...
    fn should_show_inline_completions(
        ...
        if let Some(provider) = self.edit_prediction_provider() {
            if let Some(show_edit_predictions) = self.show_inline_completions_override {
    ...
    fn report_editor_event(
        ...
        let copilot_enabled = all_language_settings(file, cx).edit_predictions.provider
